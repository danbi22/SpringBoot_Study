...
// -> 상속하는 하위 클래스는 BaseTimeEntity가 정의하는 컬럼들을 갖게 됨(실제 DB에 생성됨).

@EntityListeners(AuditingEntityListener.class)
// -> main 메서드를 갖고 있는 메인 클래스에서 JPA Auditing 기능이 활성화(@EnableJpaAuditing)되어 있는 경우에
// -> 엔터티가 삽입/수정되는 시간이 자동으로 기록되도록 하기 위해서
public class BaseTimeEntity {
	
	// 엔티티 클래스의 필드 이름은
	// 데이터베이스 테이블의 컬럼 이름과 같거나, 컬럼 이름을 camel 표기법으로 변환한 이름으로 작성
	// (예) 테이블 created_time - 클래스 createTime
